...
use super::primitives::Primitives::{self, Int, Str, HexStr, Real, Cmd, EOF};
...
pub struct Lexer<'a> {
    ...
    known_commands: Option<HashMap<&'a [u8], &'a [u8]>>,
...
macro_rules! ch {
    ($f: expr) => { $f.ok_or_else(|| LexerError(line!()))? };
}
...
impl<'a> Lexer<'a> {
    ...
    fn next_char_or_error(&mut self, line: u32) -> Result<u8, LexerError> {
        self.next_char().ok_or_else(|| LexerError(line))
    }

    pub fn get_number(&mut self) -> Result<Primitives, LexerError> {
        ...
        if num.fract() == 0.0 {
            return Ok(Int(num as i64));
        } else {
            return Ok(Real(num));
        }
    }

    pub fn get_string(&mut self) -> Result<Primitives, LexerError> {
        ...
        // str_buf :: vec<u8>
        return Ok(Str(str_buf));
    }

    pub fn get_name(&mut self) -> Result<Primitives, LexerError> {
    ...
    pub fn get_hex_string(&mut self) -> Result<Primitives, LexerError> {
        ...
        return Ok(HexStr(str_buf));
    }

    pub fn get_obj(&mut self) -> Result<Primitives, LexerError> {
    ...
    pub fn peek_obj(&mut self) -> Result<Primitives, LexerError> {
