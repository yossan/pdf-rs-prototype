...
macro_rules! get_str {
    ($obj:expr) => { $obj.get_str().ok_or_else(|| ParserError(line!()))? };
}

macro_rules! get_cmd {
    ($obj:expr) => { $obj.get_cmd().ok_or_else(|| ParserError(line!()))? };
}
...
impl<'a> XRef<'a> {
    pub fn new(stream: Stream<'a>, startxref: u64, password: Option<String>) -> XRef {
        XRef {
            stream: stream,
    ...
    fn read_xref(&mut self) {
        ...
        while self.startxref_queue.len() > 1 {
    ...
    fn process_xreftable(&mut self, mut parser: Parser) -> Result<Primitives, ParserError> {
    ...
    fn read_xreftable(&mut self, parser: &mut Parser) -> Result<Primitives, ParserError> {
